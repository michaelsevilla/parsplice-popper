...
workload, system, and parameter permutation.  Fortunately,
Section~\S\ref{sec:results} shows that the keyspace size and activity is
structured. so rather than finding thresholds by hand again for every cluster
size, growth rate, and temperature, in this section we use machine learning to
inform the Mantle policy engine.
...
plateaus of steady read requests. It also plicks different timestamps for the
...
our benchmarked optimal threshold for both the high watermark value chosen to
