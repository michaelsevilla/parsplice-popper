...
def plot_bar_cpu(dname, dname0):
    ...
    for metric, ax, means, error, scale, color in plots:
        graph = 0 
        for directory in [dname0, dname]:
            for i in range(0,9):
                df = pd.read_table(directory + "/node-" + str(i) + "." + metric + ".out",
                                   header=None, sep=(","), skiprows=13,
                                   names=("seconds", metric))
                df = df[df[metric] != 0]
                means.append((df[metric].mean())/scale)
            ...
            ax.bar(x + graph, means, width=0.3, yerr=error, label=directory.split('-')[2],
                   error_kw=dict(ecolor="black", capthick=4))
            ax.set_xticks(x)
            ax.set_xticklabels(["Splicer", "PersistentDB", "InMemoryDB",
                                "Worker", "Worker", "Worker", "Worker",
                                "Worker", "Worker", "Worker"], rotation=45)
